...
permut_deltas <- foreach(i = 1:1000, .combine = "cbind") %dopar% {
  ...
  permut_beta = matrix(nrow = nrow(betaBlood.m.adj), ncol = ncol(betaBlood.m.adj))
  ...
  for(age_group in unique(metadata$AgeGroup)){
    ...
    if(length(idx) >= 4){
      ...
      permut_beta[, twin1] = betaBlood.m.adj[, twin1]
      permut_beta[, twin2] = betaBlood.m.adj[, shuffle]
...
twin_deltas <- abs(betaBlood.m.adj[, seq(1, ncol(betaBlood.m.adj), 2)] - betaBlood.m.adj[, seq(2, ncol(betaBlood.m.adj), 2)])
...
load("permut_blood_CGI_deltas_within_ageGroup.RData")
...
CGI <- probe_islands[probe_islands$Relation_to_Island == "Island" & probe_islands$Probe_ID %in% rownames(betaBlood.m.adj), ]
...
bp <- barplot(colSums(dmCGI_MZ), las = 2, col = alpha("#b2c9c9", 0.3), border = NA,
              cex.axis = 1.1, cex.names = 1.1, xpd = TRUE, ylim = c(0, 800))
...
points(x = bp, y = unlist(lapply(DMR_MZ, nrow)), type = "b", col = "#b17769", 
       cex = 1.5)
...
dat <- data.frame(DMP_count = withinPair_DMP_counts,
                  #DMR_count = unlist(lapply(DMR_MZ, nrow)),
