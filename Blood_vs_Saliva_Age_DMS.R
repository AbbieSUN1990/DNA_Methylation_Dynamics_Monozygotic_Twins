...
###########################step1: Data Overview
#################PCA
df_pca <- pca(X = t(betaAll.m), ncomp = 2, scale = TRUE, center = TRUE)

sampleInfo <- as.data.frame(PhenoTypesAll.lv)
sampleInfo$PC1 <- df_pca$x[, "PC1"]
sampleInfo$PC2 <- df_pca$x[, "PC2"]
rownames(sampleInfo) <- colnames(betaAll.m)

ggplot(sampleInfo, aes(x = PC1, y = PC2, colour = Tissue, size = Age, shape = Population)) +
  geom_point() +
  theme_bw() +
  ...
  scale_color_manual(values = c("#F7CCC6", "#CACDE8")) +
  scale_shape_manual(values = c(20, 18, 8))
dev.off()

##################PVCA
source("PVCA.R")
PVCA(exprSet = betaAll.m, meta = sampleInfo[, 1:9])

############remove YNs_a13(Yi) and YNs_b13 (Yi) from saliva
betaAll.m.rmYi <- betaAll.m[, !colnames(betaAll.m) %in% c("YNs_a13", "YNs_b13")]
sampleInfo <- sampleInfo[!rownames(sampleInfo) %in% c("YNs_a13", "YNs_b13"), ]

############batch effect correction related to Sample_Plate
betaAll.m.rmYi.logit <- logit(betaAll.m.rmYi)
betaAll.m.logit.adj <- ComBat(dat = betaAll.m.rmYi.logit, batch = sampleInfo$Sample_Plate, mean.only = TRUE)
betaAll.m.adj <- invlogit(betaAll.m.logit.adj)

#########PCA with corrected beta value
df_pca <- pca(X = t(betaAll.m.adj), ncomp = 2, scale = TRUE, center = TRUE)
sampleInfo$PC1 <- df_pca$x[, "PC1"]
sampleInfo$PC2 <- df_pca$x[, "PC2"]

ggplot() +
  geom_point(data = sampleInfo[sampleInfo$Tissue == "blood", ], 
             aes(x = PC1, y = PC2, fill = Age), shape = 21, 
             size = 5, stroke = 0.5, colour = "#79021c") +
  scale_fill_gradient(low = "#f9e5e5", high = "#79021c") +
  new_scale_fill() +
  geom_point(data = sampleInfo[sampleInfo$Tissue == "saliva", ],
             aes(x = PC1, y = PC2, fill = Age), shape = 21,
             size = 5, stroke = 0.5, colour = "#695980") +
  scale_fill_gradient(low = "#d4bbd8", high = "#695980") +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        axis.text = element_text(colour = "black", size = 11),
        axis.title = element_text(colour = "black", size = 12),
...
#########PVCA with batch-corrected beta value
PVCA(exprSet = betaAll.m.adj, meta = sampleInfo[, 1:9])

####ggradar
data <- matrix(c("All", 33.263, 14.649, 12.73, 5.634, 4.399, 1.45, 1.429, 1.062, 1.062), nrow = 1)
colnames(data) <- c("group", "Tissue", "Age", "Sentrix ID", "Population", "Sample Plate", "Sentrix Position", "Sex", "Twins Type", "Sample Well")
...
tmp <- data.frame(x = rep(1, 9), y = rep(1.5, 9), 
                  group = paste0("g", 1:9))

anno <- ggplot()+
  geom_bar(data = tmp, aes(x, y, fill = group), stat = "identity", position = "dodge") +
  scale_fill_manual(values = alpha("#ba897f", seq(1, 0.1, length.out = 9)))+
  ylim(-6, 1.5) +
...
age_DMS <- age.test.polynomials.1.f(
  Mat = betaAll.m.adj, Ages = Age, verbose = F, pvals = T, PCUTOFF = 0.05)
...
############################step3: Compare age pattern between blood and saliva
##################################Blood:Age
#############Age-dependent CpG sites
FP <- median(apply(permut[, seq(2, ncol(permut), 2)], 2, function(x){sum(x < 0.01)}))
FDR <- FP/sum(age_DMS[, "P-value"] < 0.01)
age_DMS <- age_DMS[age_DMS[, "P-value"] < 0.01, ]
blood_vs_saliva_age <- betaAll.m.adj[rownames(betaAll.m.adj) %in% rownames(age_DMS), ]

blood.cl <- which(sampleInfo$Tissue == "blood")
saliva.cl <- which(sampleInfo$Tissue == "saliva")

############Test for differential expression between blood and saliva
########Blood as reference
RefSpecies <- blood.cl
Species2 <- saliva.cl

#multiple regression test for differential expression in age-related genes
Ref_blood_diff <- tryCatch(species.test.multiregression.1.f(
  E = blood_vs_saliva_age, x = Age, RefSpecies = RefSpecies, Species2 = Species2,
  EACH = TRUE, Ready.Bm = age_DMS[, "Model"]), error = function(e) NA)

all.inv.count <- cut(Age, breaks = 8, right = FALSE)

blood.inv.count <- table(cut(Age[blood.cl], breaks = 8, right = FALSE))

permut <- foreach(i = 1:1000, .combine = 'cbind') %dopar% {

  blood = c()
  saliva = c()
  for(i in 1:length(blood.inv.count)){
    pos = which(all.inv.count == names(blood.inv.count)[i])
    pos.blood = sample(pos, blood.inv.count[i], replace = FALSE)
    pos.saliva = setdiff(pos, pos.blood)
    blood = c(blood, pos.blood)
    saliva = c(saliva, pos.saliva)
  }

  tissue.sm <- blood_vs_saliva_age[, c(blood, saliva)]

  Exp_diff <- tryCatch(species.test.multiregression.1.f(
    E = tissue.sm, x = Age[c(blood, saliva)], RefSpecies = RefSpecies,
    Species2 = Species2, EACH = TRUE), error = function(e) NA)

  return(Exp_diff)
}

FP <- median(apply(permut[, grep("P-value", colnames(permut))], 2, function(x){sum(x < 0.001)}))
FDR <- FP/sum(Ref_blood_diff[, "P-value"] < 0.001)
Ref_blood_diff <- Ref_blood_diff[Ref_blood_diff[, "P-value"] < 0.001, ]

############Test for differential expression between blood and saliva
########Saliva as reference
RefSpecies <- saliva.cl
Species2 <- blood.cl

#multiple regression test for differential expression in age-related genes
Ref_saliva_diff <- tryCatch(species.test.multiregression.1.f(
  E = blood_vs_saliva_age, x = Age, RefSpecies = RefSpecies, Species2 = Species2,
  EACH = TRUE, Ready.Bm = age_DMS[, "Model"]), error = function(e) NA)

all.inv.count <- cut(Age, breaks = 8, right = FALSE)

saliva.inv.count <- table(cut(Age[saliva.cl], breaks = 8, right = FALSE))

permut <- foreach(i = 1:1000, .combine = 'cbind') %dopar% {

  blood = c()
  saliva = c()
  for(i in 1:length(saliva.inv.count)){
    pos = which(all.inv.count == names(saliva.inv.count)[i])
    pos.saliva = sample(pos, saliva.inv.count[i], replace = FALSE)
    pos.blood = setdiff(pos, pos.saliva)
    blood = c(blood, pos.blood)
    saliva = c(saliva, pos.saliva)
  }
...
FP <- median(apply(permut[, grep("P-value", colnames(permut))], 2, function(x){sum(x < 0.001)}))
FDR <- FP/sum(Ref_saliva_diff[, "P-value"] < 0.001)
Ref_saliva_diff <- Ref_saliva_diff[Ref_saliva_diff[, "P-value"] < 0.001, ]
 
blood_vs_saliva_age <- betaAll.m.adj[rownames(betaAll.m.adj) %in% union(rownames(Ref_blood_diff), rownames(Ref_saliva_diff)), ]

m <- matrix(c(1, 1, 2, 3, 4, 5), 2, 3, byrow = T); layout(m);
par(mar = c(3, 3, 3, 1), cex.lab = 1.2, cex.axis = 1.1, mgp = c(1, 0.2, 0), tck = -0.02)

#### Hierarchical clustering
dist <- 1 - cor(t(blood_vs_saliva_age), method = "pearson")
...
for(i in 1:length(height)){
  ...
  sig.eg = lapply(unique(group), function(grp){
    tryCatch(getMappedEntrezIDs(sig.cpg = names(group[group == grp]),
                                all.cpg = rownames(blood_vs_saliva_age),
                                array.type = "EPIC",
...
plot.data <- blood_vs_saliva_age[rownames(blood_vs_saliva_age) %in% names(memb)[memb == cl], ]
plot.data <- scale(t(plot.data), center = TRUE, scale = TRUE)
plot.data <- data.frame(cg = rowMeans(plot.data), age = Age, tissue = sampleInfo$Tissue)

fitB <- lm(cg ~ ns(age, df = 3), plot.data[plot.data$tissue == "blood", ])
predB <- predict(fitB, newdata = list(age = ageGrid), interval = "confidence")

fitS <- lm(cg ~ ns(age, df = 3), plot.data[plot.data$tissue == "saliva", ])
predS <- predict(fitS, newdata = list(age = ageGrid), interval = "confidence")

ggplot() +
  geom_point(data = plot.data, aes(x = age, y = cg, colour = tissue), 
             size = 4, shape = 21, stroke = 0.5) +
  geom_line(aes(x = ageGrid, y = predB[, 1]), linewidth = 1, colour = "#79021c") +
  geom_ribbon(aes(ymin = predB[, 2], ymax = predB[, 3], x = ageGrid),
              fill = alpha("#79021c", 0.2)) +
  geom_line(aes(x = ageGrid, y = predS[, 1]), linewidth = 1, colour = "#695980") +
  geom_ribbon(aes(ymin = predS[, 2], ymax = predS[, 3], x = ageGrid),
  ...
  scale_colour_manual(values = c("#79021c", "#695980")) +
...
bg <- getMappedEntrezIDs(sig.cpg = rownames(betaAll.m.adj),
                         array.type = "EPIC",
                         genomic.features = c("TSS200", "TSS1500", "1stExon", "5'UTR"))$sig.eg

ageGene <- ageGene[ageGene$Gene_ID %in% bg, ]

annoGene <- getMappedEntrezIDs(sig.cpg = rownames(blood_vs_saliva_age),
                               array.type = "EPIC",
                               genomic.features = c("TSS200", "TSS1500", "1stExon", "5'UTR"))$sig.eg
...
pie(c(nrow(blood_vs_saliva_age), nrow(betaAll.m.adj) - nrow(blood_vs_saliva_age)),
...
bg <- getMappedEntrezIDs(sig.cpg = rownames(blood_vs_saliva_age),
                         array.type = "EPIC",
                         genomic.features = c("TSS200", "TSS1500", "1stExon", "5'UTR"))$sig.eg
...
enrich$Group <- factor(enrich$Group, levels = paste0("C", c(1, 2, 6, 3, 4, 5)))

ggplot(enrich, aes(x = Group, y = Term)) +
  geom_tile(aes(fill = prop), colour = NA) +
  scale_fill_gradient2(low = "#BBDEEC",high = "#a06f6a", breaks = c(seq(0, 0.6, 0.1))) +
