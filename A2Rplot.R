...
"A2Rplot.hclust" <- function(
  ...
  if(only.tree){
    if(is.null(members)) plot(0,type="n",xlim=c(0.5,n.indiv+.5), ylim=c(bottom,x$height[nn]), xaxs="i", axes=FALSE, xlab="",ylab="",cex=1.6,font=2) 
    else                 plot(0,type="n",xlim=c(0.5,sum(members)+.5), ylim=c(bottom,x$height[nn]), xaxs="i", axes=FALSE, xlab="",ylab="",cex=1.6,font=2)
  ...
  if(show.labels){
    ...
    par(mar=c(0,0,0,4))
    par(srt=90)
    obs.labels <- x$labels[x$order]
    if(is.null(members)) {
      plot(0,type="n",xlim=c(0.5,n.indiv+.5), ylim=c(0,1), xaxs="i", axes=FALSE, xlab="",ylab="",cex=1.6,font=2) 
      text(1:n.indiv            , 0, obs.labels, pos=4, col=col.down[groups.o],cex=1.6,font=2)
    }
    else{
      plot(0,type="n",xlim=c(0.5,sum(members)+.5), ylim=c(0,1), xaxs="i", axes=FALSE, xlab="",ylab="",cex=1.6,font=2) 
      xo <-   members[x$order]
      text(cumsum(xo)-xo/2, 0, obs.labels, pos=4, col=col.down[groups.o],cex=1.6,font=2)
    }
    par(srt=0)
    if(boxes){
      box()
  ...
  if(!missing(fact.sup)){
    ...
    plot(0,type="n",
         ...
         ylim=c(0,n.levels), 
         xaxs="i", yaxs="i",axes=FALSE, xlab="",ylab="",cex=1.6,font=2) 
    ...
    par(mar=c(1,0,0,0))
    plot(0,type="n",xlim=c(0,1), ylim=c(0,1), xaxs="i", axes=FALSE, xlab="",ylab="",cex=1.6,font=2) 
    text(.5,.5,deparse(substitute(fact.sup)),cex=1.6,font=2)
    if(boxes){
      box()
    }
  }
